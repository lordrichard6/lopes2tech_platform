...
import { getClients } from '@/app/admin/offers/actions';
...
interface CreateOfferDialogProps {
    // When used from client page, pass client info directly
    clientId?: string;
    clientName?: string;
    clientEmail?: string;
...
type Step = 'client' | 'select' | 'review' | 'preview';
...
export function CreateOfferDialog({ 
    ...
    const hasClient = Boolean(propClientId);
    const [step, setStep] = useState<Step>(hasClient ? 'select' : 'client');
    
    // Client selection state (when not passed as prop)
    ...
    const clientName = propClientName || selectedClient?.name || '';
    const clientEmail = propClientEmail || selectedClient?.contact_email || '';
    const clientCompany = propClientCompany || selectedClient?.company_name || '';
    ...
    // Fetch clients when dialog opens (if no client passed)
    useEffect(() => {
        if (open && !hasClient && clients.length === 0) {
            fetchClients();
        }
    }, [open, hasClient, clients.length]);
    ...
    const resetForm = () => {
        setStep(hasClient ? 'select' : 'client');
        ...
        <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) resetForm(); }}>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {step === 'client' && 'Choose a client for this proposal.'}
                        {step === 'select' && `Creating proposal for ${clientCompany || clientName}`}
                        ...
                        <div className="space-y-4">
                                ...
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        {loadingClients ? (
                                            ...
                                            <span>{selectedClient.company_name || selectedClient.name}</span>
                                        ) : (
                            ...
                            {selectedClient && (
                                <div className="p-4 border rounded-lg bg-muted/20">
                                    <div className="font-medium">{selectedClient.company_name || selectedClient.name}</div>
                                    {selectedClient.company_name && (
                                        <div className="text-sm text-muted-foreground">{selectedClient.name}</div>
                                    )}
                                    {selectedClient.contact_email && (
                                        <div className="text-sm text-muted-foreground">{selectedClient.contact_email}</div>
                                    )}
                                </div>
                            )}
                        </div>
                ...
                <DialogFooter className="gap-2">
                    ...
                    {step === 'select' && (
                        <>
                            <Button variant="outline" onClick={() => hasClient ? setOpen(false) : setStep('client')}>
                                {hasClient ? 'Cancel' : 'Back'}
                            </Button>
                            <Button
